...
class MinhasDisciplinasController < ApplicationController
  ...
  def cadastrar_professor_disciplina
    ...
    if @disciplina.turmas.exists?(professor_id: @professor.id, semestre: semestre)
      redirect_to gerenciar_disciplina_path(@disciplina), alert: 'Este professor já leciona esta disciplina neste semestre.'
      return
    end
  ...
  def cadastrar_aluno_disciplina
    ...
    if @aluno.matriculas.exists?(turma_id: @turma.id)
      redirect_to gerenciar_disciplina_path(@turma.disciplina), alert: 'Este aluno já está matriculado nesta turma.'
      return
    end

    @matricula = @aluno.matriculas.create!(turma: @turma)

    redirect_to gerenciar_disciplina_path(@turma.disciplina),
