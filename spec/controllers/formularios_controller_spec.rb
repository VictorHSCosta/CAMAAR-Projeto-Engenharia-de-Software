...
RSpec.describe FormulariosController, type: :controller do
  ...
  describe 'POST #responder_formulario method functionality' do
    ...
    it 'uses verificar_acesso_aluno as a private method' do
      # Since it's a private method, check if it exists as a private method
      expect(controller.private_methods).to include(:verificar_acesso_aluno)
    end
    ...
    it 'can create text responses using private method' do
      # Create a valid opcao to satisfy foreign key constraint
      opcao = create(:opcoes_perguntum, pergunta: pergunta_texto)
      ...
      expect do
        controller.send(:criar_resposta_texto, pergunta_texto, 'Minha resposta', uuid)
  ...
  describe 'formulario response creation scenarios' do
    ...
    it 'handles subjetiva pergunta type' do
      pergunta = create(:perguntum, template: template, tipo: :subjetiva)
      uuid = 'test-uuid'
      
      # Mock criar_resposta_texto to avoid foreign key issues
      expect(controller).to receive(:criar_resposta_texto).with(pergunta, 'Resposta subjetiva', uuid)
      
      # Simulate the logic from responder_formulario
      case pergunta.tipo
      when 'subjetiva'
        controller.send(:criar_resposta_texto, pergunta, 'Resposta subjetiva', uuid)
      end
    end

    it 'handles multipla_escolha pergunta type' do
      ...
      # Mock criar_resposta_opcao to test logic
      expect(controller).to receive(:criar_resposta_opcao).with(pergunta, opcao1.id.to_s, uuid)
      expect(controller).to receive(:criar_resposta_opcao).with(pergunta, opcao2.id.to_s, uuid)
      
      # Simulate the logic from responder_formulario
      opcoes = [opcao1.id.to_s, opcao2.id.to_s]
      case pergunta.tipo
      when 'multipla_escolha'
        opcoes.each do |opcao_id|
          controller.send(:criar_resposta_opcao, pergunta, opcao_id, uuid)
        end
      end
    end

    it 'handles verdadeiro_falso pergunta type' do
      pergunta = create(:perguntum, template: template, tipo: :verdadeiro_falso)
      opcao = create(:opcoes_perguntum, pergunta: pergunta)
      uuid = 'test-uuid'
      
      expect(controller).to receive(:criar_resposta_opcao).with(pergunta, opcao.id.to_s, uuid)
  ...
  describe 'private methods' do
    ...
    describe '#criar_resposta_texto' do
      ...
      it 'creates text response when text is present' do
        expect {
          controller.send(:criar_resposta_texto, pergunta, 'Minha resposta', 'uuid123')
        }.to change(Respostum, :count).by(1)
        
        resposta = Respostum.last
        expect(resposta.resposta_texto).to eq('Minha resposta')
        expect(resposta.uuid_anonimo).to eq('uuid123')
      end

      it 'does not create response when text is blank' do
        expect {
          controller.send(:criar_resposta_texto, pergunta, '', 'uuid123')
        }.not_to change(Respostum, :count)
      end

      it 'does not create response when text is nil' do
        expect {
          controller.send(:criar_resposta_texto, pergunta, nil, 'uuid123')
        }.not_to change(Respostum, :count)
      end
    end
    ...
    describe '#criar_resposta_opcao' do
      ...
      it 'creates option response when opcao_id is present' do
        expect {
          controller.send(:criar_resposta_opcao, pergunta, opcao.id, 'uuid123')
        }.to change(Respostum, :count).by(1)
      ...
      it 'does not create response when opcao_id is blank' do
        expect {
          controller.send(:criar_resposta_opcao, pergunta, '', 'uuid123')
        }.not_to change(Respostum, :count)
      end

      it 'does not create response when opcao_id is nil' do
        expect {
          controller.send(:criar_resposta_opcao, pergunta, nil, 'uuid123')
        }.not_to change(Respostum, :count)
      end
    end
    ...
    describe '#verificar_acesso_aluno' do
      ...
      context 'when user is not aluno' do
        ...
        it 'redirects to root path' do
          ...
          expect(controller).to have_received(:redirect_to).with(root_path, alert: 'Acesso negado.')
        end
      end

      context 'when aluno does not have matricula for turma' do
        before do
          matricula.destroy
        end
        ...
        it 'redirects to evaluations path' do
          allow(controller).to receive(:redirect_to)
          result = controller.send(:verificar_acesso_aluno)
          expect(controller).to have_received(:redirect_to).with(evaluations_path, alert: 'Você não tem acesso a este formulário.')
        end
      end

      context 'when aluno has access' do
  ...
  describe 'private methods' do
    ...
    describe '#verificar_acesso_aluno' do
      ...
      it 'allows access when aluno is enrolled in turma' do
        controller.send(:verificar_acesso_aluno)
        expect(flash[:alert]).to be_nil
      end

      it 'redirects when aluno is not enrolled' do
        formulario_other_turma = create(:formulario, template: template, turma: create(:turma))
        controller.instance_variable_set(:@formulario, formulario_other_turma)
        expect(controller).to receive(:redirect_to).with(evaluations_path, alert: 'Você não tem acesso a este formulário.')
        controller.send(:verificar_acesso_aluno)
      end

      it 'redirects for non-aluno users' do
        allow(controller).to receive(:current_user).and_return(admin_user)
        formulario_other_turma = create(:formulario, template: template, turma: create(:turma))
        controller.instance_variable_set(:@formulario, formulario_other_turma)
    ...
    describe '#gerar_uuid_anonimo' do
      ...
      it 'generates consistent UUID for same user, formulario, and turma' do
        uuid1 = controller.send(:gerar_uuid_anonimo, aluno_user)
        uuid2 = controller.send(:gerar_uuid_anonimo, aluno_user)
        expect(uuid1).to eq(uuid2)
      end
      ...
      it 'uses SHA256 digest format' do
        uuid = controller.send(:gerar_uuid_anonimo, aluno_user)
        expected_uuid = Digest::SHA256.hexdigest("#{aluno_user.id}-#{formulario_aluno.id}-#{turma.id}")
    ...
    describe '#criar_resposta_texto' do
      ...
      it 'creates text response successfully' do
        # Create a valid opcao to satisfy foreign key constraint
        opcao = create(:opcoes_perguntum, pergunta: pergunta)
